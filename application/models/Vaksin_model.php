...
class Vaksin_model extends CI_Model
{
    ...
    public $table = 'vw_vaksin';
    public $column_order = array(null, 'kd_vaksin', 'vaksin', 'ket_vaksin', 'stat_vaksin', 'tgl_buat', null); //set column field database for datatable orderable
    public $column_search = array('kd_vaksin', 'vaksin', 'ket_vaksin', 'stat_vaksin', 'tgl_buat'); //set column field database for datatable searchable just firstname , lastname , address are searchable
    public $order = array('kd_vaksin' => 'desc'); // default order
    ...
    public function edit_vaksin($kd_vaksin, $vaksin, $ket_vaksin, $status)
    {
        ...
        $query = $this->db->query("SELECT * FROM tb_vaksin_kary WHERE kd_vaksin='" . $kd_vaksin . "' AND id_perusahaan=" . $id_perusahaan . " AND id_vaksin <> " . $id_vaksin);
        if (!empty($query->result())) {
            return 203;
        }

        $query = $this->db->query("SELECT * FROM tb_vaksin_kary WHERE vaksin='" . $vaksin . "' AND id_perusahaan=" . $id_perusahaan . " AND id_vaksin <> " . $id_vaksin);
        if (!empty($query->result())) {
            return 204;
        }
