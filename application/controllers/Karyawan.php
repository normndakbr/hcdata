...
ganti addsimpernew() di karyawan controller : 
    ...
    public function editSimper()
    {
        ...
        if ($cekauth == 501) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                // id_jenis_izin_tambang
                $id_jenis_izin = htmlspecialchars($this->input->post("idJenisIzin", true));
                // no_reg
                $new_no_reg = htmlspecialchars($this->input->post("editNoReg", true));
                // tgl_expired
                $new_tgl_exp = htmlspecialchars($this->input->post("editTglExp", true));

                // id_sim_kary
                $id_sim_kary = $this->kry->get_sim_kary_by_idkary($id_karyawan);
                // id_sim
                $new_id_sim = htmlspecialchars($this->input->post("editJenisSIM", true));
                // tgl_exp_sim
                $new_tgl_exp_sim = htmlspecialchars($this->input->post("editTglExpSIM", true));

                if ($id_jenis_izin == 2) {
                    if ($new_tgl_exp == "" && $new_tgl_exp_sim == "") {
                        echo json_encode(array("statusCode" => 422, "status" => "error", "pesan" => "", "pesan1" => "Tanggal mulai wajib diisi", "pesan2" => "Tanggal akhir wajib diisi"));
                        return;
                    } else if ($new_tgl_exp == "") {
                        echo json_encode(array("statusCode" => 422, "status" => "error", "pesan" => "", "pesan1" => "Tanggal mulai wajib diisi", "pesan2" => ""));
                        return;
                    } else if ($new_tgl_exp_sim == "") {
                        echo json_encode(array("statusCode" => 422, "status" => "error", "pesan" => "", "pesan1" => "", "pesan2" => "Tanggal akhir wajib diisi"));
                        return;
                    } else if ($new_tgl_exp > $new_tgl_exp_sim) {
                        echo json_encode(array("statusCode" => 422, "status" => "error", "pesan" => "", "pesan1" => "", "pesan2" => "Isi tanggal akhir dengan benar"));
                        return;
                    }
                }
                ...
                echo json_encode(array(
                    ...
                    "message" => "id_jenis_izin => " . $id_jenis_izin . ", new_no_reg => " . $new_no_reg . ", new_tgl_exp => " . $new_tgl_exp . ", id_sim_kary => " . $id_sim_kary . ", new_id_sim => " . $new_id_sim . ", new_tgl_exp_sim => " . $new_tgl_exp_sim
                ));
                return;
            }
        }
